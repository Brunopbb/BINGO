...
#!/usr/bin/env python
#--------------------------------------------------------------------------------------
# Adapted by Mathews Chirindo - South African Radio Astronomy Observatory
...
# DESCRIPTION :
#
# This script uploads a spectrometer fpg file to one or more SKARAB systems that contain one 
...
p.add_option('-f', '--freq', dest='central_freq', type='int',default=160,
help='Set the central frequency. default is 800MHz.')
p.add_option('-d', '--dec', dest='decimation', type='int',default=16,
help='Set the decimation. default is 32.')
p.add_option('-c', '--counter_check', dest='counter_checkpoint', type='int',default=1000000000,
help='Set the maximum number of saved files. default is 10.')
p.add_option('-p', '--folder_out', dest='folder_out', type='str', help='Set the output folder.')
p.add_option('-i', '--ip', dest='ip', type='str',default='10.42.1.152',
help='Set the ip. default is 10.42.1.152.')
...
def get_data():
    ...
    a_1=struct.unpack('>32768L',skarabs[0].read('mem_left_0_0',32768*4,0))
    #a_2=struct.numpy.array(interleave_a,dtype=numpy.float)unpack('>16384Q',skarabs[0].read('mem2',16384*8,0))
...
def accumulating_data():
    acc_n, interleave_a = get_data()	
    meusdados.append(interleave_a)
...
try:
    ...
    skarab_ips = [my_skarab_ip] # Other options: ['10.0.7.3']; etc
    ...
    # 2.7 SET THE DDC CENTRE FREQUENCY16384
    ...
    frequence_mhz = options.central_freq

    #######################

    mhz_multiplier = 1000000
    channels_ddc_centre_freq = frequence_mhz * mhz_multiplier
    ...
    sys.stdout.flush()
    for i in range(skarab_num):
        skarabs[i].write_int('acc_len',opts_acc_len)
    ...
    aux_meusdados = []
    #average_aux_meusdados = []

    # Define a data atual e cria o diretório correspondente
    ...
    try:
        while True:
            my_acc_n, my_interleave_a = get_data()
            counter += 1
            ...
            if ((counter % 4) == 0):
                ...
                # Salva realtime.npy (sobrescreve)
                realtime_path = os.path.join(output_dir, 'realtime.npy')
                average_meusdados = np.mean(aux_meusdados, axis=0)
                ...
                npz_file = os.path.join(output_dir, "Averages_%s.npz" % minute_str)

                if os.path.exists(npz_file):
                    old_data = np.load(npz_file)
                    data_dict = dict(old_data.items())
                    old_data.close()
                else:
                    data_dict = {}

                data_dict[timestamp_key] = average_meusdados
                print(np.array(average_meusdados))
        ...
        file_name = str(options.folder_out) + '/Jordany_dados_Fc%d_Dec%d_Ti%s_Tf%s' % (
            central_freq_MHZ, options.decimation, inicio_str, final_str)
